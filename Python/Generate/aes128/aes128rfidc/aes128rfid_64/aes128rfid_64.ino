...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 64; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 64) {
            memset(&cardID[idLength], 0, 64 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 64);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 64);
        displayData();
