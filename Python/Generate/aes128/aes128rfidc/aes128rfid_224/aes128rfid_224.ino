...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 224; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 224) {
            memset(&cardID[idLength], 0, 224 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 224);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 224);
        displayData();
