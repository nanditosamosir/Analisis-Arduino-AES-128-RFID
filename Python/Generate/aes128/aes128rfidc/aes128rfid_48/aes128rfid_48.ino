...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 48; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 48) {
            memset(&cardID[idLength], 0, 48 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 48);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 48);
        displayData();
