...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 80; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 80) {
            memset(&cardID[idLength], 0, 80 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 80);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 80);
        displayData();
