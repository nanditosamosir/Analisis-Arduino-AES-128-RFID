...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 144; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 144) {
            memset(&cardID[idLength], 0, 144 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 144);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 144);
        displayData();
