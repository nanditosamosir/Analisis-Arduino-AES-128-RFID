...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 112; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 112) {
            memset(&cardID[idLength], 0, 112 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 112);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 112);
        displayData();
