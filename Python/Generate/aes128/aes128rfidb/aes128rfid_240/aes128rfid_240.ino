...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 240; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 240) {
            memset(&cardID[idLength], 0, 240 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 240);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 240);
        displayData();
