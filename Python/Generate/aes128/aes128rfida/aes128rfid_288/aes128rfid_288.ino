...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 288; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 288) {
            memset(&cardID[idLength], 0, 288 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 288);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 288);
        displayData();
