...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 192; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 192) {
            memset(&cardID[idLength], 0, 192 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 192);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 192);
        displayData();
