...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 32; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 32) {
            memset(&cardID[idLength], 0, 32 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 32);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 32);
        displayData();
