...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 16; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 16) {
            memset(&cardID[idLength], 0, 16 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 16);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 16);
        displayData();
