...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 256; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 256) {
            memset(&cardID[idLength], 0, 256 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 256);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 256);
        displayData();
