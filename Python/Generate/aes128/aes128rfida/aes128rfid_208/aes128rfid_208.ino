...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 208; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 208) {
            memset(&cardID[idLength], 0, 208 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 208);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 208);
        displayData();
