...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 272; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 272) {
            memset(&cardID[idLength], 0, 272 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 272);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 272);
        displayData();
