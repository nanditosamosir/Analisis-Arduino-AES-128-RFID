...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 160; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 160) {
            memset(&cardID[idLength], 0, 160 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 160);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 160);
        displayData();
