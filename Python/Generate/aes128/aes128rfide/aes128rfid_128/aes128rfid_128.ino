...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 128; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 128) {
            memset(&cardID[idLength], 0, 128 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 128);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 128);
        displayData();
