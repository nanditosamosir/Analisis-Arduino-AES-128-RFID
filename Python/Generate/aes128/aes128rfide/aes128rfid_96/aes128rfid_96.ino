...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 96; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 96) {
            memset(&cardID[idLength], 0, 96 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 96);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 96);
        displayData();
