...
void loop() {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
        ...
        for (byte i = 0; i < idLength && i < 176; i++) {
            cardID[i] = rfid.uid.uidByte[i];
        }

        if (idLength < 176) {
            memset(&cardID[idLength], 0, 176 - idLength);
        }
        ...
        encryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), cardID, 176);
        decryptAndPrint(&aes128, keyAES128, sizeof(keyAES128), ciphertext, 176);
        displayData();
